...
def os_name(name_os, name_original, name_reemplazo):
    if name_os == "posix":
        ...
        if arquitectura == "64bit":
            procesado_gs = f"""gs -sDEVICE=pdfwrite -dCompatibilityLevel=1.5
            -dPDFSETTINGS=/ebook -dNOPAUSE -dQUIET -dBATCH
            -sOutputFile={name_reemplazo}""".split()
            procesado_gs.append(name_original)
            return procesado_gs
        ...
        comando.insert(0, f"gswin64c.exe")
        return comando
...
def abrir_fichero():
    try:
        ...
        filename = filedialog.askopenfiles(filetypes=[("Pdf files", "*.pdf")], title="Seleccione documentos PDFs", initialdir=os.path.expanduser('~'))
        for nombre_file in filename:
            ...
            if os.path.exists(reemplazo):
                ...
                if msg_sn is True:
#                    os_name(os.name, nombre_file.name, reemplazo)
                    subprocess.run(os_name(os.name, nombre_file.name, reemplazo))
...
def command_about():
    ...
    or improvement proposal on Github.'
    """
